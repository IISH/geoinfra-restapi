...
module.exports = function(req, res, next) {
    ...
    if (req.query.children) {
        hgSource = _(hg.fetch({path:'/search?related=hg:liesIn&related.id='+req.query.name+'&geometry=false',method:'GET'}));
    } else {
        hgSource = _(hg.fetch({path:'/search?name='+req.query.name+'&geometry=false', method: 'GET'}));
    }
